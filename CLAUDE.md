...
| `python tools/render_save.py --save <pack>/<save_id>` | after every turn (load-bearing: keeps markdown in sync with JSON) |
| `python tools/lint_save.py --save <pack>/<save_id>` | after every turn, or before trusting a save for load / save/load continuity checks |
| `python tools/inspect_save.py --save <pack>/<save_id>` | on demand to summarize a save |
